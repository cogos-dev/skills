...
- `cogos-harness`'s `memory-janitor` `Stop` hook + skill — blocks the turn once when a project's auto-memory `MEMORY.md` crosses a size threshold (default 18800 bytes) and points the pilot at the `memory-janitor` skill instead of letting it compact inline. The skill dispatches a bounded background `haiku` lane against a standing directive, grades the result deterministically with a bundled checker (five checks: size window with an overtrim floor, link-target set with occurrence counts, bullet inventory including linkless bullets, section headers, per-entry clause retention), and repairs on defects for up to 2 rounds. No-ops for any project without auto-memory.
