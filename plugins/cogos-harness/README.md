...
| `threads-warn.py` | `UserPromptSubmit` (`*`) | Reads the threads registry (below), runs each open thread's resolution predicate under a hard per-predicate timeout plus an overall wall-clock budget, and emits a compact warning block **only** for orphaned threads (unresolved and past `expected_by`). Silent for an empty registry, an all-healthy registry, a missing state file, or a corrupt one — see `skills/threads/SKILL.md` and the module docstring for the full silence contract. |
